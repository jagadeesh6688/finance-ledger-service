...
transactionSchema.methods.reject = async function (approverId, reason) {
  ...
  logger.info(`Transaction ${this.transactionId} rejected by ${approverId}: ${reason}`);
};
 ...
 */
transactionSchema.methods.createDoubleEntry = async function (partnerTransaction) {
  ...
  logger.info(`Double-entry created: ${this.transactionId} <-> ${partner.transactionId}`);
